...
p1=ggplot(tmp2)+aes(Easting,Northing,fill=dist_mainland)+geom_raster()+tmp.theme+scale_fill_gradient(low="black", high="white")
print(p1,vp=viewport(layout.pos.row=1,layout.pos.col=1))
p2=ggplot(tmp2)+aes(Easting,Northing,fill=dist_shelf)+geom_raster()+tmp.theme+scale_fill_gradient(low="black", high="white")
print(p2,vp=viewport(layout.pos.row=1,layout.pos.col=2))
p3=ggplot(tmp2)+aes(Easting,Northing,fill=ice_conc)+geom_raster()+tmp.theme+scale_fill_gradient(low="black", high="white")
...
p5=ggplot(tmp2)+aes(Easting,Northing,fill=dist_edge)+geom_raster()+tmp.theme+scale_fill_gradient(low="black", high="white")
...
#overlay photographs over grid cells, calculating total area and # spotted seals surveyed in each
...
Spotted=Points[["SPOTTED"]]
Spotted.count=rep(0,n.cells)
for(i in 1:n.points){
  Area.photo[which.cell[i]]=Area.photo[which.cell[i]]+Area.m2[i]
  Spotted.count[which.cell[i]]=Spotted.count[which.cell[i]]+Spotted[i]
}
...
Spotted=Spotted.count[Mapping]

#Add some color to plots - green if 1-2 seals, yellow if 3-4 seals, orange if 5-9 seals and red for 10+
plot(Cur.grid[Mapping[which(Spotted==1 | Spotted==2)],],add=TRUE,col="yellow")
plot(Cur.grid[Mapping[which(Spotted==3 | Spotted==4)],],add=TRUE,col="orange")
plot(Cur.grid[Mapping[which(Spotted>4)],],add=TRUE,col="violet")
plot(Cur.grid[Mapping[which(Spotted>9)],],add=TRUE,col="red")
